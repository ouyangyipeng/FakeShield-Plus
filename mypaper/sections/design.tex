...
We apply INT8 quantization to DTE-FDM using the bitsandbytes library~\citep{bitsandbytes}. The quantization is applied through the \texttt{load\_in\_8bit=True} flag in \texttt{load\_pretrained\_model}:
...
\begin{enumerate}
    ...
    \item Fine-tuning on the SD\_inpaint dataset~\citep{fakeshield2025}
\end{enumerate}
